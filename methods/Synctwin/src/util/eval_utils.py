...
def summarize_B(mat_b, show_plot=True):
    ...
    wrong_proba_sd = np.std(
        np.sum(mat_b[:, (mat_b.shape[1] // 2) :], axis=1)
    ) / np.sqrt(mat_b.shape[0])

    entropy = np.mean(np.sum(mat_b * np.log(mat_b + 1e-9), axis=1)) * -1
    entropy_sd = np.std(np.sum(mat_b * np.log(mat_b + 1e-9), axis=1)) / np.sqrt(
        mat_b.shape[0]
    )
...
def effect_mae_from_w(mat_w, n_units, y_control, y_full, treatment_effect):
    ...
    sc_mae_effect_sd = torch.std(
        torch.abs(treatment_effect - sc_effect_est)
    ).item() / np.sqrt(mat_w.shape[0])
