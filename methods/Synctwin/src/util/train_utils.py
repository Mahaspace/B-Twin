...
from methods.Synctwin.src.util import batching_utils as batching
...
def pre_train_reconstruction_prognostic_loss(
    ...
    optimizer = optim.Adam(
        list(dec.parameters()) + list(enc.parameters()) + list(dec_Y.parameters())
    )
...
def train_cfr(
    ...
    for itr in range(1, niters + 1):
        ...
        if itr % test_freq == 0:
            with torch.no_grad():
                loss = cfr(
                    x_full_val, t_full_val, mask_full_val, y_full_val, y_mask_full_val
                )
...
def load_pre_train_and_init(
    ...
    batch_ind_full,
    model_path="models/sync/{}.pth",
    init_decoder_Y=False,
):
    enc = nsc.encoder
    dec = nsc.decoder

    enc.load_state_dict(
        torch.load(model_path.format("encoder.pth"), map_location=DEVICE)
    )
    dec.load_state_dict(
        torch.load(model_path.format("decoder.pth"), map_location=DEVICE)
    )

    if init_decoder_Y:
        dec_Y = nsc.decoder_Y
        dec_Y.load_state_dict(
            torch.load(model_path.format("decoder_Y.pth"), map_location=DEVICE)
        )
...
def load_nsc(
    nsc, x_full, t_full, mask_full, batch_ind_full, model_path="models/sync/{}.pth"
):
    nsc.load_state_dict(torch.load(model_path.format("nsc.pth"), map_location=DEVICE))
    with torch.no_grad():
        C = nsc.get_representation(x_full, t_full, mask_full)
        nsc.update_C0(C, batch_ind_full)
    nsc.eval()
