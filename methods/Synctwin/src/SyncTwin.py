...
# import src.GRUD as GRUD
import torch
import torch.nn as nn
import torch.nn.functional as F

from methods.Synctwin.src import GRUD as GRUD
...
class SyncTwin(nn.Module):
    def __init__(
        ...
        if reduce_gpu_memory:
            ...
            init_B = (torch.ones(n_unit + n_treated, n_unit, dtype=dtype)).to(
                device
            ) * 1.0e-4
        self.B = nn.Parameter(init_B)
        self.C0 = torch.zeros(
            n_unit, self.encoder.hidden_dim, dtype=dtype, requires_grad=False
        ).to(device)
    ...
    def prognostic_loss(self, B_reduced, y_batch, y_control, y_mask):
        ...
        return (
            torch.sum(((y_batch - y_hat) ** 2) * y_mask.unsqueeze(-1))
            / torch.sum(y_mask)
            * self.lam_prognostic
        )
...
class RegularEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, bidirectional=True, device=DEVICE):
        ...
        self.lstm = nn.LSTM(input_dim, hidden_dim, bidirectional=bidirectional).to(
            device
        )
        if bidirectional:
...
# class RegularEncoderMLP(nn.Module):
#     def __init__(self, input_dim, hidden_dim, device=DEVICE):
#         super(RegularEncoderMLP, self).__init__()
#         self.input_dim = input_dim
#         self.device = device
...
#         # B, Dh
#         hidden = torch.sum(h * attn_weight.unsqueeze(-1), dim=0)
#         return hidden

# class RegularDecoderMLP(nn.Module):
#     def __init__(self, hidden_dim, output_dim, max_seq_len, device=DEVICE):
#         super(RegularDecoderMLP, self).__init__()
#         self.device = device
#         self.max_seq_len = max_seq_len
#         self.mlp = nn.Sequential(
#             nn.Linear(hidden_dim, hidden_dim),
...
#     def forward(self, hidden, t, mask):
#         # hidden: B, Dh
...
class RegularEncoderMLP(nn.Module):
    def __init__(
        self, n_features, seq_len, embedding_dim=100, hidden_dims=[128], device=DEVICE
    ):
        super(RegularEncoderMLP, self).__init__()
...
class RegularDecoderMLP(nn.Module):
    def __init__(
        ...
        # Final layer to output the reconstructed sequence
        layers.append(nn.Linear(input_dim, seq_len))
    ...
    def forward(self, hidden, t, mask):
        ...
        output = output.view(output.size(0), self.seq_len, 1)
