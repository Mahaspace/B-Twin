...
def outcome_pre(x, W, t, alpha_=0.05, beta=0.2):
...
def outcome_post(x, W, t, alpha_=0.05, beta=0.2):
    ## complex nonlinear outcome after treatment
    group_effect = (
        x * 0.5 * (W**2)
        + alpha_ * t * (W > 0.5)
        + 3 * np.sin(0.1 * t) * W
...
def generate_simulation(
    ...
    for i in range(n_units):
        for t in range(n_time):
            if t < treatment_time:
                Outcome[i, t] = outcome_pre(
                    AR_1[t], W[i], t, alpha_=alpha
                ) + np.random.normal(0, sigma)
            else:
                Outcome[i, t] = outcome_post(
                    AR_1[t], W[i], t, alpha_=alpha
                ) + np.random.normal(0, sigma)
    if bias:
